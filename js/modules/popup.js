...
import { bodyLockStatus, bodyLock, bodyUnlock, bodyLockToggle, lockPaddingElements, scrollbarGutterValue, supportsScrollbarGutter, lockPaddingValue } from '../modules/functions.js';
...
// * в данном случае не надо т.к у header нет position: fixed
	...
	constructor(options) {
		let config = {
			...
			on: {
				...
				afterClose: function () {
					// * очисть форму после закрытия popup - Я
			...
			function (e) {
				...
				if (buttonOpen) {
					...
					if (supportsScrollbarGutter && scrollbarGutterValue === 'stable') {
						//* добавляем popup padding-right = ширине полосы прокрутки
				...
				if (buttonClose || (!e.target.closest(`.${this.options.classes.popupContent}`) && this.isOpen)) {
					...
					if (supportsScrollbarGutter && scrollbarGutterValue === 'stable') {
						...
						// удалять с задержкой 500ms, чтобы не скакал popup
						setTimeout(() => {
